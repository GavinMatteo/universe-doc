...
Fashion stores succeed when they lead with strong visuals, create desire through lifestyle imagery, and make product discovery feel effortless. This guide walks you through the recommended section order and key configuration choices.
...
- Upload a separate mobile image for each slide. Fashion photography is usually portrait-oriented and crops poorly on desktop if not adapted.
- Set **Overlay protection** to 20-40% so text is readable without hiding the image.
...
**Key settings:**
- Set **Products per row** to 3 on desktop and 2 on mobile.
...
## 💡 Tips for fashion stores

- Keep the color scheme minimal. Fashion photography works best against light or dark neutral backgrounds, not colorful sections.
- Use the Comparison Table section on collection pages to compare different products in the same line (for example, different fabric options or fits).
- For multi-category stores (menswear, womenswear, accessories), use the Collection List section after the hero to direct visitors to their area.
