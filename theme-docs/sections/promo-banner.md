...
📌 **When to use it:** Use it on the homepage or campaign pages to communicate a time-sensitive offer or a key brand benefit.
...
| **Big promo text** | The large number displayed prominently in the section, for example 70 for a 70% off offer. |
| **Small promo text** | A short symbol after the number, for example %, x, or +. |
...
| **Image position** | Controls whether the image appears on the left or right side of the text column. |
| **Choose image** | The background image for this section on desktop. |
| **Choose image (mobile)** | A separate background image for mobile. If left blank, the desktop image is used. |
| **Choose video** | A background video file for desktop. Supports MP4 format. The video loops silently. |
