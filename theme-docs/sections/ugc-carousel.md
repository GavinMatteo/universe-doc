...
| **Show socials** | Shows social platform icons on UGC cards to indicate the content source. |
| **Content alignments (mobile)** | Controls the horizontal alignment of the content block within the section: Left, Center, or Right. |
...
| **Enable card gradient overlay** | Adds a soft gradient overlay at the bottom of each card to improve text readability over the media. |
...
| Setting | What it does |
|---|---|
| **Choose video from your library** | A background video file for desktop. Supports MP4 format. The video loops silently. |
| **Or embed video from Youtube or Vimeo url** | An external video URL from YouTube or Vimeo. |
| **Or show an image** | The background image for this section on desktop. |
| **Card link URL** | The URL this UGC card links to when clicked. |
| **Video alt text** | Alternative text describing the video content for screen readers and accessibility. |
| **Enable video autoplay** | When enabled, the video starts playing automatically as the card comes into view. |
| **Video cover-poster image** | A cover image shown before the video starts playing. |
| **Heading** | The heading or label for this block element. |
| **Paragraph** | The text content for this block. |
| **Show paragraph only on hover (desktop)  ** | When enabled, the paragraph text is hidden until the visitor hovers over the card on desktop. |
...
| **Label background color** | The background color of the label badge. |
| **Label text color** | The text color of the label badge. |
...
Make sure the video URL is a valid public link. For uploaded files, check the format is MP4.
